          ...
          <div className="columns-2 md:columns-3 lg:columns-4 gap-4 [column-fill:_balance]">
            {(items ?? []).map((it: any) => (
              <div key={it.id} className="mb-4 break-inside-avoid overflow-hidden rounded-2xl group">
                <ModeImage src={it.image_url} alt={it.title ?? "Galeri"} mode={it.display_mode} className="w-full aspect-[4/3] bg-muted" imgClassName="transition-transform duration-700 group-hover:scale-105" />
              </div>
            ))}
          </div>
        )}
